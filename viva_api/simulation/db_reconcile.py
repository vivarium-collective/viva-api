...
# Repo root = two levels up from viva_api/simulation/db_reconcile.py
...
async def _marker_hpcrun_chain_current(conn: AsyncConnection) -> bool:
...
async def _marker_task_table(conn: AsyncConnection) -> bool:
    """True once the ``task`` table exists (d7e2f4a6c8b0, viva-api#631).

    Satisfied for free by a create_all-bootstrapped database — ORMTask is a Base
    table, so create_db creates it at startup, which is the property every marker
    in this list needs."""
