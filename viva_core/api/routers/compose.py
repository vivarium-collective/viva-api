...
from viva_core.compose.handlers import (
...
async def get_results(simulation_id: int) -> FileResponse | StreamingResponse:
    ...
    if hpc_run is not None and hpc_run.job_backend != JobBackend.SLURM.value:
        # Batch: the run never zips anything -- it writes straight to S3, and a chained analysis
        # job's results land under the SAME experiment prefix, so streaming every object under it
        # captures both with a single dispatch.
        settings = get_settings()
        prefix = layout.experiment_prefix(settings.s3_output_prefix, experiment_id)
        file_service = _require_files()
        archive_name = f"{experiment_id}.tar.gz"
        return StreamingResponse(
            stream_prefix_tar_gz(file_service, prefix, experiment_id),
    ...
    # A backend that builds its own archive (SLURM: the HPC-side zip, fetched over SSH) hands it
    # over; how it fetches is the backend's, not this route's (P3d-4d-2).
    backend = _require_sim(ComputeBackend.SLURM) if hpc_run is not None else _require_sim()
    try:
        local_path = await backend.results_archive(experiment_id)
    except Exception:
        logger.exception("Failed to fetch compose results for simulation %s", simulation_id)
        raise HTTPException(502, f"Failed to download results from the backend for simulation {simulation_id}")
...
async def get_biomodels_identifiers(
    ...
    from viva_core.contrib.sysbio.biomodels_service import BiomodelsService
...
async def _submit_biomodel(
    ...
    from viva_core.contrib.sysbio.biomodel_documents import make_biomodel_document
    from viva_core.contrib.sysbio.biomodels_service import BiomodelsService
...
async def run_biomodels(
    ...
    """
    from viva_core.contrib.sysbio.biomodel_documents import COPASI_STEP_ADDRESS, TELLURIUM_STEP_ADDRESS
    from viva_core.contrib.sysbio.biomodels_service import BiomodelsService
