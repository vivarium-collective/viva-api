...
Companion to :mod:`viva_core.api.auth`, which reads an unverified header. This
...
def _fetch_json(url: str, timeout: float) -> dict[str, object]:
    ...
    if not isinstance(document, dict):
        raise TypeError(f"{url}: expected a JSON object, got {type(document).__name__}")
...
def subject_from_bearer(request: Request) -> str | None:
    ...
    Never raises, matching :func:`viva_core.api.auth.resolve_caller`'s contract:
