...
from viva_core.storage.file_service import ListingItem
...
def create_token() -> Token:
    return Token(
        service_file=get_core_settings().storage_gcs_credentials_file,
        scopes=[
...
async def upload_bytes_to_gcs(file_contents: bytes, s3_path: S3FilePath, token: Token) -> S3FilePath:
    ...
    async with Storage(token=token) as client:
        await client.upload(
            bucket=get_core_settings().storage_gcs_bucket, file_data=file_contents, object_name=str(s3_path)
        )
        return s3_path
...
async def get_gcs_file_contents(s3_path: S3FilePath, token: Token) -> bytes | None:
    ...
    try:
        async with Storage(token=token) as client:
            return await client.download(bucket=get_core_settings().storage_gcs_bucket, object_name=str(s3_path))
