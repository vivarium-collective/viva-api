...
# What this module asks of libsedml's objects. libsedml is a SWIG binding with no type information,
# so its classes are not named in annotations (D12); these are the few members used.
class SedModel(Protocol):
    def getSource(self) -> str: ...


class SedSimulation(Protocol):
    def getInitialTime(self) -> float: ...

    def getOutputStartTime(self) -> float: ...

    def getOutputEndTime(self) -> float: ...

    def getNumberOfPoints(self) -> int: ...


class SedDocument(Protocol):
    def getNumSimulations(self) -> int: ...

    def getSimulation(self, index: int, /) -> SedSimulation | None: ...

    def getNumModels(self) -> int: ...

    def getModel(self, index: int, /) -> SedModel | None: ...
...
def _read_sedml_doc(sedml_path: str) -> SedDocument:
    doc: SedDocument | None = libsedml.readSedMLFromFile(str(sedml_path))
    if doc is None:
        raise RuntimeError(f"libsedml returned None reading: {sedml_path}")
    return doc


def _extract_first_utc(sed_doc: SedDocument) -> UniformTimeCourseSpec:
...
def _resolve_sbml_source(sed_doc: SedDocument, sedml_dir: str, fallback: str) -> str:
...
class BiomodelsService:
    ...
    @staticmethod
    def get_metadata(biomodel_id: str) -> dict[str, object]:
        """Return raw metadata dict for a single BioModel."""
        import biomodels

        meta: object = biomodels.get_metadata(biomodel_id)
        for method in ("dict", "model_dump"):
            dump = getattr(meta, method, None)
            if callable(dump):
                dumped: object = dump()
    ...
    def load_biomodel(biomodel_id: str, stable_dir: Path) -> BiomodelLoadResult:
        ...
        with tempfile.TemporaryDirectory(prefix=f"biomodel_{biomodel_id}_") as tmp:
            # Fetch files
            def _fetch(entry: object, out_dir: str) -> str:
