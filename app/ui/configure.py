...
def _():
    ...
    return HPCFilePath, config_service


@app.cell
def _(config_service):
    # this should be <WORKSPACE_DIR>/vEcoli
    wd_root = config_service.vecoli_root_dir()
...
def _(config_service):
    ...
    config_default = config_service.default_config()
    return (config_default,)


@app.cell
def _(config_default):
    config_default
    return


@app.cell
def _(config_default, config_service, generate_config_fields, mo):
    ...
    def get_parca_ui(parca_dict):
        ...
        for key in parca_dict.keys():
            ...
            if isinstance(val_default, str):
                ...
                parca_ui_dict[key] = mo.ui.number(label=key_label, start=1, stop=64, step=1, value=val_default)
            new_genes_dir = config_service.new_genes_dir()

            new_gene_options = [parca_options_default["new_genes"]]

            new_gene_options = new_gene_options + next(new_genes_dir.remote_path.walk())[1]
    ...
    parca_form_fields = generate_config_fields(parca_options_exposed, get_parca_ui(parca_options_default))
    ...
    **ParCa options:**
    ...
    fields_list_parca = ["{" + str(element) + "}" for element in parca_form_fields.keys()]
...
@app.cell
def _(config_service, mo, variant_switch):
    ...
    variant_modules = config_service.list_variants()
    variant_select = None
...
def _(generate_config_fields, mo):
    ...
    def variant_form_new_gene():
        ...
        variant_fields_ui = {
            ...
            "exp_start": mo.ui.number(label=variant_fields_label["exp_start"], start=1, stop=10, step=1),
            "exp_stop": mo.ui.number(label=variant_fields_label["exp_stop"], start=1, stop=10, step=1),
            "exp_num": mo.ui.number(label=variant_fields_label["exp_num"], start=1, stop=10, step=1),
            "trl_eff": mo.ui.slider(label=variant_fields_label["trl_eff"], start=0.0, stop=1.0, step=0.01),
            "rel_exp_adj_list": mo.ui.text(label=variant_fields_label["rel_exp_adj_list"]),
            "rel_trl_eff_adj_list": mo.ui.text(label=variant_fields_label["rel_trl_eff_adj_list"]),
        }
        ...
        **Variant Options:**
        ...
        fields_list = ["{" + str(element) + "}" for element in variant_form_fields.keys()]
        ...
        variant_form = mo.ui.batch(html=variant_form_md, elements=variant_form_fields).form(
            show_clear_button=True, submit_button_label="Apply", bordered=False
        )
    ...
    def load_variant_new_gene(variant_options_input):
        ...
        if variant_options_input["knockout"]:
            ...
            variant_options_dict["new_gene_internal_shift_variable_strength"]["knockout_gen"] = knockout_opt
    ...
    def load_variant_internal_shift(variant_options_input):
        ...
        if variant_options_input["knockout"]:
            ...
            knockout_opt["value"] = []
            knockout_opt["value"].append(variant_options_input["knockout_gen"])
            variant_options_dict["new_gene_internal_shift"]["knockout_gen"] = knockout_opt
...
def _(config_default, mo, parca_switch, variant_switch):
    ...
    field_ui_mapping = {
        "experiment_id": mo.ui.text(
            value="sms_experiment_" + str(uuid.uuid4())[:4], label=f"{field_label_mapping['experiment_id']}"
        ),
        ...
        "generations": mo.ui.number(label=f"{field_label_mapping['generations']}", start=1, stop=40, step=1, value=5),
        "n_init_sims": mo.ui.number(label=f"{field_label_mapping['n_init_sims']}", start=1, stop=100, step=1, value=35),
        ...
        "fail_at_max_duration": mo.ui.checkbox(label=f"{field_label_mapping['fail_at_max_duration']}"),
...
@app.cell
def _(HPCFilePath, mo, os, wd_root):
    # analysis options
    analysis_switch = mo.ui.checkbox(label="Run analysis", value=True)

    def get_analysis_modules(analysis_type, root: HPCFilePath):
        dir_analysis = root.remote_path / "ecoli" / "analysis"
        # dir_analysis = os.path.join(root, "ecoli", "analysis")
        modules = os.listdir(str(dir_analysis / analysis_type))
    ...
    def get_analysis_menu(analysis_type, default: list[str] | None = None):
        menu = mo.ui.multiselect(
            options=get_analysis_modules(analysis_type, wd_root), label=f"Analysis ({analysis_type})", value=default
        )
        return menu
...
def _(analysis_switch, mo):
    ...
    if analysis_switch.value:
        analysis_type_select = mo.ui.multiselect(
            ...
            label="Analysis type",
            value=["multiseed", "multigeneration"],
        )
...
def _(analysis_type_select, get_analysis_menu):
    ...
    if not isinstance(analysis_type_select, type(None)):
        ...
        if "multigeneration" in analysis_type_select.value:
            analysis_menu_multigeneration = get_analysis_menu("multigeneration", default=["ribosome_crowding"])
        if "multiseed" in analysis_type_select.value:
            analysis_menu_multiseed = get_analysis_menu("multiseed", default=["ribosome_spacing"])
...
def _(
    ...
    if not isinstance(analysis_menu_single, type(None)):
        if len(analysis_menu_single.value) > 0:
            analysis_options_single = {module: {} for module in analysis_menu_single.value}
    ...
    if not isinstance(analysis_menu_multidaughter, type(None)):
        if len(analysis_menu_multidaughter.value) > 0:
            analysis_options_multidaughter = {module: {} for module in analysis_menu_multidaughter.value}
    ...
    if not isinstance(analysis_menu_multigeneration, type(None)):
        if len(analysis_menu_multigeneration.value) > 0:
            analysis_options_multigeneration = {module: {} for module in analysis_menu_multigeneration.value}
    ...
    if not isinstance(analysis_menu_multiseed, type(None)):
        if len(analysis_menu_multiseed.value) > 0:
            analysis_options_multiseed = {module: {} for module in analysis_menu_multiseed.value}
    ...
    if not isinstance(analysis_menu_multivariant, type(None)):
        if len(analysis_menu_multivariant.value) > 0:
            analysis_options_multivariant = {module: {} for module in analysis_menu_multivariant.value}
...
def _(
    ...
    config_final = config_form.value or emitter_options
    if parca_switch.value:
...
@app.cell
def _(field_ui_mapping, mo):
    config_save_name = mo.ui.text(label="Save config as: ", value=field_ui_mapping["experiment_id"].value)
    config_save_name
    return (config_save_name,)


@app.cell
def _(config_final, config_save_name, json, mo):
    save_button = mo.ui.run_button(label="save config", kind="success")
    # save_button
    export_filename = config_save_name.value
    if not export_filename.endswith(".json"):
        export_filename += ".json"

    def on_download() -> None:
        data = json.dumps(config_final, indent=2).encode("utf-8")
        print(f"Now downloading your config as {export_filename}...")
        return data
