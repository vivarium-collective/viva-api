...
| 5 | Image resolution | `<ecr>/v2ecoli:<commit>`, derived **four separate times** from the same two settings (`dispatch/batch_layer.py`, `compose/simulation_service_ray.py`, `compose/env_worker_service.py`, `simulation_service_k8s.py`) | one `EnvironmentResolver`; the *select* half of D10 (§2.3) | P2.3 | in progress — 2.3a: the model and `RegistryEnvironmentResolver` exist in `viva_core/environments/` and are pinned to say what the four derivations say; the four still derive by hand until 2.3b |
