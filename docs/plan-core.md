...
| 3f ✅ | **settings split finished; two OpenAPI documents; the lifespan runs without the SMS scheduler.** `slurm_log_base_path` — the one field 3d-2 could not move — is on `CoreSettings` (the `file_paths` ↔ `settings` cycle is broken: the path type reads settings at call time); every field a `viva_core` module reads is declared there (mypy). **Core's own document**: `viva_core/api/openapi_spec.py` writes `viva_core/api/spec/openapi_3_1_0_generated.yaml` from `create_core_app()` (47 operations, all under `/viva/v1`); `make spec` writes both; a test pins that the committed core document is current and that the application's document is the union (every core operation id is served by SMS). The SMS lifespan starts the scheduler if one exists and serves without it otherwise. **Checkpoint E2** | what unblocks the generated core client (D8) |
...
| P4d | the live feed — v2ecoli `artifact()` + call sites; sms-ecoli pin; simulator; the consumer's API asks | | | | **step 1 (viva-api) in PR #841**: `sim_id` pinned in simulation baggage (it was already injected; now guarded), the registry reads `lineage_seed` as `seed`. Steps 2–5 (v2ecoli, sms-ecoli) wait on Eran's review of the P4d order (#655); checkpoint G2 not run; folded from `plan-data-provenance.md` §9 steps 3–4 on 2026-09-26 |
...
- **2026-09-25** — **P3f: the settings split is finished, core has its own OpenAPI document, and P3 is complete.**
  ...
  The path type now reads settings at call time (inside `local_path()`, the one method that needs
  them), the field is on `CoreSettings`, and `compose/hpc_utils.py` reads through core like the rest.
  Every field a `viva_core` module reads is declared on `CoreSettings`; mypy is the proof, since a
  read of an undeclared field does not type-check. **Two documents.** `viva_core/api/openapi_spec.py`
  builds an app on bare `CoreSettings` and writes its document: 47 operations, all under `/viva/v1`
  (health, environments, compose ×16, env-worker ×30); `make spec` writes it beside the application's.
  A test pins that the committed core document is current and that **the application's document is
  the union** — every core operation id is served by SMS, at core's prefix for core's own two routes
  and at `/compose/v1` / `/env-worker/v1` for the routers SMS mounts where its callers expect them.
  The application's document did not change by a byte. The core document's `version` is FastAPI's
  default `0.1.0`: core has no version line of its own yet, which is D8's question (a generated client
  needs one). **The lifespan.** `main.py` refused to start without a `JobScheduler`; the scheduler is
  SMS's, and core's routes, compose and env workers do not need it, so the app now starts without one
  and says so in the log. **Proof:** `make check` clean twice; suite 2298 passed. P3 is complete:
  everything the phase named — `create_core_app()`, the container, the settings split, the compose and
  env-worker packages and routes in core, two documents — is on `main`; checkpoint E2 deploys what
  landed after 0.9.155 (3d-4d-2a/b, 3e, 3f).
