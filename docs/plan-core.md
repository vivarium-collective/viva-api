...
- **P2.1 — carve, move-only, one concern per PR**, leaves first: config interpretation →
  ...
  **No re-exports for module-level functions** (settled by cut 1): a moved function's importers
  ...
  | 5 | ParCa and the caches → `ray/parca.py` (`RayParcaMixin`): cache URIs, the ParCa / new-gene / variant / upstream commands, their three submit methods, per-seed founder-cache staging | **this PR** | 512 (3,840 → 3,328) | open |
...
| P2.1 | carve `simulation_service_ray.py`, one concern per PR (Batch engine → core). Cut 1, config interpretation — #705 · cut 2, Batch engine → `viva_core/backends/batch.py` — #706 · cut 3, tasks + the shared base layer — #707 (merged `4173ca26`) · cut 4, build — #712 (merged `8229315a`) · cut 5, ParCa — **this PR** | no bump: deploys with the rest of P2.1 at checkpoint C | — | — | **in progress** — cuts 1–3 merged 2026-09-19; nothing deployed (checkpoint C) |
