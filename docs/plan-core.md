...
| 3d-4d-2b | **the env-worker router is core's** (30 routes, 978 lines): needs core's `auth` (`require_caller` / `resolve_caller`, which read `viva_api.config`) and the `owner_instance` setting; the relay's process-wide `runner` and `registry` are what 3e's container replaces | the env-worker half |
