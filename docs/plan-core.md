...
| **`/viva/v1` is served by the pod and not routed by the ALB** (found at D2, 2026-09-21): the listener has a rule per top-level prefix and none for `/viva`, so core's routes reach PTools' 404 page through the tunnel. Nothing uses them yet | sms-cdk `internal-alb-stack.ts` (sms-cdk#56: one additive `ListenerRule`; dev, then prod before prod serves `/viva/v1`) | merge + `cdk deploy smsvpctest-internal-alb` (Jim's go); smoke Tier 0 `core` then flips FAIL → PASS |
