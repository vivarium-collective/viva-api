...
| 3d-4c-2 | **the runner and what loads it**: `run_pbg.py` (1,165 lines, 32 `Any`, 7 domain terms) and `render_nf.py` (loads it; 8 `Any`); then core supplies the runner itself and the two `runner_source=` seams get a core default. `handlers.py` (`run_compose_v2ecoli`, 7 terms) is 3d-4d's with the routes | the largest file, on both counts |
