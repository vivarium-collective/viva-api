...
- **P2.3 — the environment model and its *select* half** (no database, no build). D10 says
  ...
  **Sequence** (one concern per PR, as in P2.1; nothing merges without Jim's say-so):
  ...
  | **D** | **Checkpoint D**: bump, deploy (the API roll is what makes `CORE_RUNTIME_IMAGE` take effect), Tier 0 + 1 with `--environment runtime --require-aws`, and a like-for-like queue-to-running time for `task` and `compose` on both images | the deploy that judges all of P2.3 |
