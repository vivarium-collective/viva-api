...
| F2 | P3g | code only — the interim `/viva/v1` mounts, core's capability route, the `contract` check | Tier 0 incl. `contract` on both prefixes; `vwb smoke` with a W1 workbench; may ride F's image |
...
| `app/gui.py` and `app/ui/dashboard.py` are byte-identical (1,587 lines each) | `app/` | delete `app/gui.py` at P3g |
