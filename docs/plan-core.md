...
| 3d-4b | **the `Any`-free pass** over what has no domain terms: `database_service` (5), `job_monitor` (6), `env_worker_relay` (8), `render_nf` (8), `tables_orm` (0), `env_worker_schemas` (0; one description string names a model's config) — then they move (`viva_core/db/`, `services/`, `env_worker/`, `compose/`) | D12 is a glob: a module arrives `Any`-free or not at all |
