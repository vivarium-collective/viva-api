...
| 10 | **viva-api#742 (Eran, 2026-09-21): push the split to "one object, one run, one record" — and one surface?** Six ranked changes and five questions for Jim. Two strategies are drafted for the two of them to decide between: [`strategy-core-incremental.md`](strategy-core-incremental.md) (Eran's `viva_core` end state, the SMS contract permanent) and [`strategy-core-direct.md`](strategy-core-direct.md) (no facades; dated breaking releases across three repos). #776 (the `JobStore` seam) is consistent with both | before P4a | **open — Jim and Eran** |
...
- **2026-09-22** — **#742 is a decision for the two humans, and it now has two documents instead of a thread.**
  ...
  one environment concept, one generic run surface) with the SMS contract permanent (the 29 workbench
  paths, the 3 PTools calls, the aliases); **B, direct** — the same core with no facades, every SMS
  surface dated and removed on a clock, with the coordinated workbench / PTools / atlantis releases and
  the prod catch-up that implies. Open question 10; nothing in P0–P3d is undone by either. #776 fits both.
