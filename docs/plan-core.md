...
- **P2.3 — the environment model and its *select* half** (no database, no build). D10 says
  ...
  **Sequence** (one concern per PR, as in P2.1; nothing merges without Jim's say-so):
  ...
  | 2.3d-2 | **make the image pullable by Batch** — Jim's decision: make the ghcr package public, or mirror the tag into a new ECR repository — then one trial pull on dev's container queue, then `CORE_RUNTIME_IMAGE` in dev's `api.env` | an org-package visibility change or new registry infrastructure: not a line in a code PR |
  | 2.3d-3 | compose (its container path) and env workers may name an environment. **Checkpoint D**: deploy, Tier 1 with `--task-environment runtime --require-aws`, and the queue-to-running time against the 250–320 s measured on the science image | the rest of the API change, and the deploy that judges all of P2.3 |
...
| Where Batch pulls the runtime image from is **undecided, and now the blocker for 2.3d-2**: `build-core-runtime.yml` pushed `ghcr.io/vivarium-collective/viva-core-runtime:0.1.0` (`sha256:356405729d9b09ce3aa9a4e18b2f43a4dee207760d2bc45cf5a87434b80c9ace`, amd64 + arm64) on 2026-09-21. A new ghcr package is **private** (an anonymous manifest request answers 403; `sms-api`'s answers 200), Batch has no pull secret, and GitHub offers no API to change an organisation package's visibility | deployment; Jim | **(a)** make the package public in the org's package settings — it holds Python, the open-source engine, the AWS CLI and core's entrypoint, nothing of any application — and try one pull from the Batch VPC; or **(b)** a new ECR repository `viva-core-runtime` in the shared account, mirrored from ghcr: what every image Batch pulls today looks like, no public exposure, one more thing to keep in step |
...
| P2.3 | the environment model and its *select* half (D10): one resolver for four image derivations; then the core runtime image. 2.3a the model + `RegistryEnvironmentResolver` (`viva_core/environments/`, no caller changed) — #748 · 2.3b the four derivations ask it (`common/site_environments.py`) — #749 · 2.3c the core runtime image + core's container entrypoint (`Dockerfile-core-runtime`, `viva_core/runtime/`) — #750 · 2.3d-1 a task may name an environment (`TaskRunRequest.environment`) | | | | **in progress** — 2.3a, 2.3b, 2.3c, 2.3d-1 done (none deployed). The runtime image is **pushed** (`viva-core-runtime:0.1.0`, private). **Waiting on Jim:** how Batch pulls it (2.3d-2). Then 2.3d-3 and checkpoint D |
