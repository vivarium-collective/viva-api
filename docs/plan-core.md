...
| 3d-2 | **the 14 settings compose and env-worker read become `CoreSettings` fields** (same names, same environment variables, same defaults — SMS's `Settings` inherits them), and compose reads them through core's accessor. Proof: `Settings.model_fields` identical before and after | settings first: nothing can move while it imports `viva_api.config` |
| 3d-3 | **compose's four `viva_api.dependencies` lookups** (database, file service, SSH session — two of them lazy) arrive as constructor arguments | services second |
...
| P3 | settings, DI, app factory. 3a `create_core_app()` boots alone; SMS includes core's router under `/viva/v1` | | | | **in progress** — 3a, 3b, 3c done (not deployed); 3d-1 done (compose's ParCa staging is a hook); next 3d-2 (the 14 settings compose reads become `CoreSettings` fields) |
