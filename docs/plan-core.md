...
- **P2.1 — carve, move-only, one concern per PR**, leaves first: config interpretation →
  ...
  **No re-exports for module-level functions** (settled by cut 1): a moved function's importers
  ...
  | 3 | tasks → `ray/tasks.py` (`RayTasksMixin`), on two prerequisites every later mixin shares: `ray/image_paths.py` (in-image path constants, a leaf) and `ray/batch_layer.py` (`RayBatchLayer`, the service's delegations to the engine) | **this PR** | 597 (4,581 → 3,984) | open |
...
| P2.1 | carve `simulation_service_ray.py`, one concern per PR (Batch engine → core). Cut 1, config interpretation — #705 · cut 2, Batch engine → `viva_core/backends/batch.py` — #706 · cut 3, tasks + the shared base layer — **this PR** | no bump: deploys with the rest of P2.1 at checkpoint C | — | — | **in progress** — cuts 1–2 merged 2026-09-19, cut 3 open; nothing deployed (checkpoint C) |
