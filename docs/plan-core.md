...
| 3d-4b-3 | **the five move**: `database_service`, `tables_orm` → `viva_core/db/`; `job_monitor` → `viva_core/services/`; `env_worker_relay`, `env_worker_schemas` → `viva_core/env_worker/`; the abstract `ComposeSimulationService` out of the file that keeps the SLURM class; the by-name entries leave the D12 override as core's glob takes over | the move itself |
