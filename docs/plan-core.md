...
| 3d-4d-2 | **the routes**: compose's (16) and env-worker's (30) are served by core's router; `/compose/v1` and `/env-worker/v1` stay as aliases; the SLURM results-download branch and `/curated/ecoli` stay SMS's; BioModels (`biomodels_service`, `biomodel_documents`: 19 `Any`) → `contrib/sysbio`; core's `auth` | routes last: they are what clients see |
