...
- **P2.1 — carve, move-only, one concern per PR**, leaves first: config interpretation →
  ...
  **No re-exports for module-level functions** (settled by cut 1): a moved function's importers
  ...
  | 6 | analysis → `ray/analysis.py`, a **service**: `RayAnalysisService(dispatch)` behind an `AnalysisDispatch` Protocol, plus `analysis_memory_class`, `analysis_modules_for` and their constants | #715 | 513 (3,361 → 2,848) | open |
...
| P2.1 | carve `simulation_service_ray.py`, one concern per PR (Batch engine → core). Cut 1, config interpretation — #705 · cut 2, Batch engine → `viva_core/backends/batch.py` — #706 · cut 3, tasks + the shared base layer — #707 (merged `4173ca26`) · cut 4, build — #712 (merged `8229315a`) · cut 5, ParCa — #713 (merged `1f90dd04`) · build and tasks → composed services — #714 (merged `a10ac6cf`) · cut 6, analysis as a service — #715 | no bump: deploys with the rest of P2.1 at checkpoint C | — | — | **in progress** — cuts 1–3 merged 2026-09-19; nothing deployed (checkpoint C) |
