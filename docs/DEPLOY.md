...
## 1. Three images, three different versioning schemes (and two more nothing deploys yet)
...
| `viva-core-runtime` | the tag given to its workflow | CI — `build-core-runtime.yml`, **manual `workflow_dispatch`**; refuses an existing tag | **its own line**, write-once: changes with the engine pin (`viva_core/runtime/requirements.txt`) or the entrypoint, not with sms-api. First tag: `0.1.0` (2026-09-21, `sha256:356405729d9b…`), **public** on ghcr; Batch pulls it anonymously (proven from dev's VPC). Named by `CORE_RUNTIME_IMAGE` in `<ns>/api.env` — **dev only**; not in `shared.env`, which also rolls ptools. Prod: repeat the one-job trial pull from its VPC first |
| `viva-core` | `viva_core/version.py` (core's line, D16) — the tag given to its workflow | CI — `build-core.yml`, **manual `workflow_dispatch`**; gate = `tests/core` + the in-image boot check (`scripts/core_image_check.py`); refuses an existing tag | **core's own line**, write-once. `Dockerfile-core`: viva_core served by uvicorn (`--factory viva_core.api.app:create_core_app`), **no application package in the image** (`tests/test_deploy_config.py` asserts it). **Tags `0.1.0`** (2026-09-26, `sha256:ef44499b…`, **public** on ghcr — contents are `viva_core` alone, nothing of ptools) **and `0.1.1`** (the SLURM build-row tag fix, #810; `viva_core/version.py` moves with the tag). Deployed by the UConn core overlay `viva-core-rke-dev` (plan-core §4b U3) beside the SMS `api` pod |
