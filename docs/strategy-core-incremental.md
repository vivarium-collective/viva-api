...
is a single constraint, and its reason: **the SMS surface that the workbench and PTools call today
does not change for the scope of this refactor** — not its paths, not its shapes. Not because the
surface is meant to be permanent (it was never intended to be), but to contain risk: the refactor
already moves every service, every table mapping and every dispatch mechanism, and changing what the
callers see at the same time would add a second axis of failure to every checkpoint. SMS becomes a
thin application over core, and its endpoints become facades that *map* SMS concepts onto core's,
exactly the premise the split was started under. Whether those facades are later dated and removed
(Strategy B's end state) is a **separate decision, taken after the refactor**, with its own plan.
...
|---|---|---|
| `/api/v1/simulations*`, `/api/v1/analyses*`, `/ws` | workbench, PTools, atlantis | **unchanged for this refactor** — SMS facades over core jobs, campaigns and datasets |
| `/core/v1/simulator/*`, `/core/v1/capabilities` | workbench, atlantis | **unchanged for this refactor** — the `simulator` record stays (D6, D11) and *maps to* a core environment |
| `/compose/v1`, `/env-worker/v1` | workbench | **unchanged for this refactor** — aliases of core's own routers (`/viva/v1/composites`, `/viva/v1/workers`), same code, two prefixes |
| `/api/v1/tasks`, `/api/v1/datasets` | atlantis, smoke | SMS facades over `/viva/v1/tasks`, `/viva/v1/datasets`, unchanged for this refactor |
| `/viva/v1/*` | new callers, the core CLI | core's canonical surface; may grow freely |

"Unchanged for this refactor" means: changed or removed only by a *separate* decision with its own
migration plan and its own risk budget, never as a side effect of this one. It answers #742's
question 3 this way: no external constraint forces the aliases to exist forever; they are kept
*now* because breaking a caller we ship beside is not this refactor's job. Where the plan documents
currently say "permanent" for these surfaces, that word overstated the intent and is corrected to
this.
...
| 5 | one environment concept | **P5**: the D10 resolver gains its *build* half; a `simulator` becomes a **registered environment** (`ExplicitSpec`, `repo-recipe`, write-once per D11) and the `simulator` table becomes a projection over the `core.environment` table — kept for this refactor, because `/core/v1/simulator/*` is what the workbench calls today | the `simulator` id space and its routes |
| 6 | one generic run surface; ecoli as a package | **P8–P9**: `/viva/v1/composites` is the run surface for new callers; the science model is a package (sms-ecoli) that registers composites, hooks (`runner_hooks.py`), analysis modules and layouts with core's extension points; atlantis's generic verbs delegate to the generated core CLI (D8) | `/api/v1/simulations` and the atlantis SMS verbs stay as the ecoli-shaped front door for this refactor |
| 1 | aliases as a dated phase | **deferred, not declined**: dating the four surfaces above is the decision that follows this refactor (Strategy B's first step), taken with the callers' owners; **accepted now** for anything new: a `/viva/v1` route never gets an alias | — |

Everything already merged (P0–P3d) is consistent with this table; nothing is undone.

## What this costs, and what it buys

*Costs.* Two spellings of some things for the length of the refactor, and until a later decision
removes them: `simulator` beside `environment`, `/compose/v1` beside `/viva/v1/composites`, `hpcrun`
beside `core.job` until P7. Each is a facade
...
never notice. The 15-comment thread on #742 collapses to two decisions (D14: the SMS contract is
unchanged for the scope of this refactor, and dated afterwards by a separate decision; D15:
templates as the campaign primitive at P7) and the existing sequence.
...
from F onward. A change that fails it is a contract change and needs its own decision — which, once
the refactor is done, may well be the decision to make it.
