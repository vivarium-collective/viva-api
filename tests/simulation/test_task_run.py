...
def test_submit_task_routes_large_memory_class_to_large_queue() -> None:
    ...
    with (
        patch("viva_api.simulation.dispatch._seams.get_settings", lambda: settings),
...
def test_submit_uploaded_task_stages_script_and_runs_staged_path() -> None:
    ...
    from viva_api.simulation.dispatch.image_paths import TASK_STAGE_DIR
...
def test_get_task_logs_reads_cloudwatch_stream() -> None:
    ...
    with (
        patch("viva_api.simulation.dispatch._seams.get_settings", lambda: settings),
        patch("viva_api.simulation.dispatch._seams.boto3.client", return_value=fake),
    ):
        import asyncio
